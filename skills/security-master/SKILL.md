...
- **[Auth Implementation Patterns](sub-skills/auth-implementation-patterns/SKILL.md)** — builds secure authentication and authorization from the ground up: session vs token (JWT) design, OAuth2/OIDC flows, MFA/TOTP, password storage, and RBAC. **Use when:** implementing or hardening login, OAuth/JWT issuance, 2FA, or access control (e.g. Doorkeeper/Devise/JWT/WebAuthn stacks). **Not for:** remediating a specific known auth bug (use Broken Authentication Remediation) or design-phase threat modeling (use Security Architecture & Design).

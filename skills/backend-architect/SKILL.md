...
- **[Stripe Integration](sub-skills/stripe-integration/SKILL.md)** — PCI-conscious Stripe payment flows: Checkout, subscriptions, webhook handling, idempotency, and refunds. **Use when:** implementing Stripe billing, subscription lifecycle, or webhook processing specifically. **Not for:** non-Stripe processors (use Payment Integration / PayPal Integration).
...
> 💾 **Database, SQL, ClickHouse, Redis, and vector-store skills now live in [Data Master](../data-master/SKILL.md).** Chain to it for relational/OLAP schema design, query tuning, caching, and semantic search.
- **[New Rails Project](sub-skills/new-rails-project/SKILL.md)** — Scaffold a new Ruby on Rails application with sensible defaults for database, testing, and directory conventions. **Use when:** bootstrapping a greenfield Rails service or API-only app. **Not for:** upgrading or restructuring an existing Rails app (use Rails Upgrade Assessment).
- **[Rails Upgrade Assessment](sub-skills/skill-rails-upgrade/SKILL.md)** — Analyze a Rails codebase and produce a version-by-version upgrade plan covering gem compatibility, deprecations, config changes, and risk hotspots. **Use when:** planning a Rails major/minor upgrade (e.g. to Rails 8), auditing gem/Ruby compatibility, or triaging deprecation warnings. **Not for:** creating a brand-new app (use New Rails Project).
...
### 3. API & Communication
> 🔌 **API design — pick one:** stable endpoints & type contracts → **API Design**; generate concrete endpoint specs from a domain → **API Endpoint Designer**; establish/review conventions & maturity → **API Design Principles**; author an OpenAPI 3.1 contract → **OpenAPI Spec Generation**.
...
- **[API Endpoint Designer](sub-skills/api-designer/SKILL.md)** — Generate production-ready REST endpoint specifications (resources, methods, status codes, request/response schemas, error contracts) from a described domain. **Use when:** you need concrete endpoint specs quickly from a feature description. **Not for:** high-level design theory (use API Design Principles) or type-contract design (use API Design).
- **[API Design Principles](sub-skills/api-design-principles/SKILL.md)** — Deep REST and GraphQL design guidance (resource modeling, versioning, pagination, error semantics) with checklists and best-practice references. **Use when:** establishing API conventions or reviewing an API surface against maturity standards. **Not for:** generating specific endpoint stubs (use API Endpoint Designer).
- **[OpenAPI Spec Generation](sub-skills/openapi-spec-generation/SKILL.md)** — Author and maintain OpenAPI 3.1 specifications (design-first or code-derived), with validation and SDK-generation patterns. **Use when:** documenting an HTTP API, enforcing an API contract, or generating clients/servers from a spec (e.g. rswag/Swagger workflows). **Not for:** converting a spec to a Postman collection (use Postman OpenAPI Converter).
- **[Postman OpenAPI Converter](sub-skills/postman-openapi-converter/SKILL.md)** — Convert OpenAPI 3.x / Swagger 2.0 specs into import-ready Postman Collection v2.1 JSON. **Use when:** producing a Postman collection from an existing API spec for QA or manual testing. **Not for:** authoring the spec itself (use OpenAPI Spec Generation).
...
> ⚡ **Performance — pick one:** already have a concrete slow spot → **Performance Optimization**; need measured before/after proof → **Performance Optimizer**; planning load tests / capacity from metrics → **Performance Engineering**; locating CPU/memory hotspots via flame graphs → **Performance Profiling**.
...
> 🏛️ **Architecture — pick by altitude:** system-wide style/tech decisions → **System Architecture** / **Software Architecture** (section 1); concrete multi-file blueprint for one feature → **Code Architect**; cross-cutting refactor of a large existing system → **Architect**; find shallow→deep refactor opportunities → **Improve Codebase Architecture**.

...
> **⚠️ NEVER use for implementation plan execution.** If you have an implementation plan with multiple tasks, use `subagent-driven-development` (sequential subagent-per-task with two-stage review). This skill is for **parallel investigation/debugging** — not parallel building. Building in parallel causes file conflicts, inconsistent state, and broken integrations.
